// =================================================================================
// This is auto-generated by GoFrame CLI tool only once. Fill this file as you wish.
// =================================================================================

package dao

import (
	"gowebserver/app/system/dao/internal"
)

// sysLoginHistoryDao is the manager for logic model data accessing and custom defined data operations functions management.
// You can define custom methods on it to extend its functionality as you wish.
type sysLoginHistoryDao struct {
	*internal.SysLoginHistoryDao
}

var (
	// SysLoginHistory is globally public accessible object for table sys_login_history operations.
	SysLoginHistory = sysLoginHistoryDao{
		internal.NewSysLoginHistoryDao(),
	}
)

// Fill with you ideas below.
