// ==========================================================================
// 基于gf的gcache模块,进行了轻量的service封装
// 主要是添加了缓存标签支持,可以对同类型的数据定义一个标签,清除缓存时可以按照标签分类清除.
// ==========================================================================
...
func (c *cacheTagService) cacheTagKey(key interface{}, tag interface{}) {
	...
	if c.tagKey != nil {
		...
		if err := c.cache.Set(c.tagKey, tagValue, 0); err != nil {
			g.Log().Error(err)
		}
	}
}
...
func (c *cacheTagService) Sets(data map[interface{}]interface{}, duration time.Duration, tag interface{}) {
	...
	if tag != nil {
		...
		if err := c.cache.Sets(data, duration); err != nil {
			g.Log().Error(err)
		}
	} else {
		if err := c.cache.Sets(data, duration); err != nil {
...
func (c *cacheTagService) Removes(keys []interface{}) {
	if _, err := c.cache.Remove(keys...); err != nil {
		g.Log().Error(err)
	}
}
